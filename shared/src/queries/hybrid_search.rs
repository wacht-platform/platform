...
impl Query for HybridSearchKnowledgeBaseQuery {
    ...
    async fn execute(&self, app_state: &AppState) -> Result<Self::Output, AppError> {
        ...
        tracing::info!(
            "Executing hybrid search - Query: '{}', KB ID: {}, Deployment ID: {}, Max results: {}, Min relevance: {}, Weights: vector={}, text={}",
            self.query_text,
        ...
        let results = sqlx::query_as::<_, HybridSearchKbResult>(
            ...
            SELECT 
                ...
                text_rank,
                combined_score
            FROM hybrid_search_kb_enhanced(
                $1::TEXT,                    -- query text
                $2::vector(768),             -- query embedding
                $3::BIGINT,                  -- knowledge_base_id
                $4::BIGINT,                  -- deployment_id
                $5::INT,                     -- max_results
