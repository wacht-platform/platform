...
use models::api_key::{RateLimit, RateLimitMode};
...
pub async fn check_authz(
    ...
    response_headers.insert(
        "X-Wacht-App-ID",
        safe_header_value(key_data.app_id.to_string()),
    );
    response_headers.insert(
        "X-Wacht-App-Slug",
    ...
    if all_allowed {
        let effective_limits = if let Some(scheme_slug) = &key_data.rate_limit_scheme_slug {
                ...
                .await
            {
                ...
                key_data.rate_limits.clone()
            }
        } else {
            debug!(app_slug = %key_data.app_slug, "[GATEWAY] Using app rate limits");
            key_data.rate_limits.clone()
        };
    ...
    (
        ...
        Json(AuthzCheckResponse {
            ...
            identity: Some(AuthzIdentity {
                ...
                app_id: key_data.app_id.to_string(),
