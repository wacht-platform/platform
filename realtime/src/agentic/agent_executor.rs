...
use shared::models::{
    ...
    ExecutionContextStatus, ImmediateContext, MemoryRecord, ResponseFormat, UserInputRequestState,
    WorkflowEdge, WorkflowNode, WorkflowNodeType,
};
use shared::queries::{
    GetExecutionContextQuery, GetLLMConversationHistoryQuery, GetMRUMemoriesQuery, Query,
};
...
impl AgentExecutor {
    ...
    fn extract_suggestions(&self, input_request: &Value) -> Option<Vec<String>> {
        input_request
            .get("suggestions")
            .and_then(|v| v.as_array())
            .map(|arr| {
    ...
    fn extract_validation_hints(&self, input_request: &Value) -> Option<String> {
        input_request
    ...
    fn build_platform_function_schema(
        &self,
        func_config: &shared::models::PlatformFunctionToolConfiguration,
    ) -> Result<Value, AppError> {
    ...
    async fn execute_user_input_node(
        &self,
        config: &shared::models::UserInputNodeConfig,
    ) -> Result<Value, AppError> {
        // Send user input request via channel
        {
            let user_input_request = ConversationContent::UserInputRequest {
                ...
                input_type: match &config.input_type {
                    shared::models::UserInputType::Text => "text",
                    shared::models::UserInputType::Number => "number",
                    shared::models::UserInputType::Select => "select",
                    shared::models::UserInputType::MultiSelect => "multiselect",
                    shared::models::UserInputType::Boolean => "boolean",
                    shared::models::UserInputType::Date => "date",
                }
                .to_string(),
                ...
                .await
            {
                Ok(embeddings) => {
                    ...
                    if embeddings.len() != working_memory_items.len() {
                        ...
                        // Store working memory items with their embeddings
                            ...
                            working_memory_items.iter().zip(embeddings.iter())
                        {
                            ...
                            if embedding.is_empty() {
                                tracing::warn!(
                                    "Skipping memory with empty embedding: {}",
                                    memory_content
                                );
                                continue;
                            }
                            ...
                            match self.app_state.sf.next_id() {
                                Ok(id) => {
                                    ...
                                    // Calculate importance based on content length and uniqueness
    ...
    fn get_current_workflow_state(&self) -> Option<shared::models::WorkflowExecutionState> {
        match (
            ...
            (Some(workflow_id), Some(workflow_state), Some(node_id)) => {
                Some(shared::models::WorkflowExecutionState {
                    workflow_id,
    ...
    pub async fn resume_workflow_execution(&mut self) -> Result<Value, AppError> {
        ...
        // No platform function result available yet - workflow remains pending
        Ok(json!({
            "workflow_id": workflow.id,
            "workflow_name": workflow.name,
            "execution_status": "pending",
