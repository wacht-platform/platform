...
pub async fn revoke_api_key_for_app(
    ...
    api_key_key_app::revoke_api_key_for_app(&app_state, deployment_id, app_slug, key_id, request)
        .await?;
...
pub async fn rotate_api_key_for_app(
    ...
    let key = api_key_key_app::rotate_api_key_for_app(&app_state, deployment_id, app_slug, key_id)
        .await?;
    Ok(key.into())
}
