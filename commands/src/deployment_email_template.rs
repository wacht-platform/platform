    ...
    async fn execute(self, app_state: &AppState) -> Result<Self::Output, AppError> {
        let column_name = match self.template_name {
            ...
            DeploymentNameParams::WebhookFailureNotificationTemplate => {
                "webhook_failure_notification_template"
            }
        };

        if self.template.template_data.contains("<style") {
            return Err(AppError::BadRequest(
                "Email templates cannot contain <style> tags. Please use inline styles."
                    .to_string(),
            ));
        }
        ...
        let template_json =
            serde_json::to_value(&template).map_err(|e| AppError::Serialization(e.to_string()))?;
