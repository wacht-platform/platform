...
const MEMORY_EMBEDDING_DIMENSION_768: i32 = 768;
...
pub async fn initialize_memory_table(config: &VectorStoreConfig) -> Result<(), AppError> {
    ...
    let table = crate::vector_store::open_or_create_table_in_connection(
        &conn,
        MEMORY_TABLE,
        memory_schema(),
    )
    .await?;
    ensure_memory_table_schema(&table).await?;
...
pub async fn insert_memory(
    ...
    let batch = build_memory_record_batch(std::slice::from_ref(record), embedding_dimension)?;
    table
        .add(batch)
        .execute()
        .await
...
pub async fn count_indexable_memory_rows(config: &VectorStoreConfig) -> Result<usize, AppError> {
    count_indexable_memory_rows_for_dimension(config, MEMORY_EMBEDDING_DIMENSION).await
}

pub async fn count_indexable_memory_rows_for_dimension(
...
pub async fn memory_vector_index_exists(config: &VectorStoreConfig) -> Result<bool, AppError> {
    memory_vector_index_exists_for_dimension(config, MEMORY_EMBEDDING_DIMENSION).await
}

pub async fn memory_vector_index_exists_for_dimension(
    config: &VectorStoreConfig,
    embedding_dimension: i32,
) -> Result<bool, AppError> {
    let Some(table) = crate::vector_store::open_table(config, MEMORY_TABLE).await? else {
        return Ok(false);
    };
...
pub async fn create_memory_vector_index(config: &VectorStoreConfig) -> Result<(), AppError> {
    create_memory_vector_index_for_dimension(config, MEMORY_EMBEDDING_DIMENSION).await
}

pub async fn create_memory_vector_index_for_dimension(
    config: &VectorStoreConfig,
    embedding_dimension: i32,
) -> Result<(), AppError> {
    let conn = crate::vector_store::connect_vector_store(config).await?;
...
pub async fn get_startup_memories_in_table(
    ...
    load_memories_in_table(table, deployment_id, &filter, limit, embedding_dimension).await
}
...
pub async fn load_memories_in_table(
    ...
    let mut records = parse_memory_records(&batches, deployment_id, embedding_dimension)?;
...
pub async fn search_memories_in_table(
    ...
    let filter = build_semantic_memory_filter(filters, embedding_dimension);
    let batches = table
    ...
    parse_memory_records(&batches, deployment_id, embedding_dimension)
}
...
fn build_semantic_memory_filter(filters: &MemoryQueryFilters, embedding_dimension: i32) -> String {
...
async fn ensure_memory_table_schema(table: &Table) -> Result<(), AppError> {
    let schema = table.schema().await.map_err(map_vector_store_error)?;

    if schema.field_with_name("embedding_768").is_err() {
        tracing::info!(
            table = MEMORY_TABLE,
            "Adding missing embedding_768 column to memory table"
        );
        table
            .add_columns(
                NewColumnTransform::AllNulls(Arc::new(Schema::new(vec![Field::new(
                    ...
                    DataType::FixedSizeList(
                        ...
                        MEMORY_EMBEDDING_DIMENSION_768,
                    ),
                    true,
                )]))),
                None,
...
fn build_memory_record_batch(
    ...
    let embeddings =
        build_memory_embedding_array(records, embedding_dimension, MEMORY_EMBEDDING_DIMENSION)?;
    let embeddings_768 =
        build_memory_embedding_array(records, embedding_dimension, MEMORY_EMBEDDING_DIMENSION_768)?;
...
fn parse_memory_records(
    ...
    for batch in batches {
        ...
        let embeddings = batch
            .column_by_name(embedding_column_for_dimension(embedding_dimension))
        ...
        let embeddings_legacy = batch
            .column_by_name("embedding")
            .and_then(|column| column.as_any().downcast_ref::<FixedSizeListArray>());
        let embeddings = if embeddings_legacy.is_some() && embedding_dimension == 1536 {
            embeddings_legacy.unwrap_or(embeddings)
        } else {
            embeddings
        };
...
fn read_embedding_at_row(
    embedding_values: &Float32Array,
    row_idx: usize,
    dimension: i32,
) -> Vec<f32> {
    let start = row_idx * dimension as usize;
    let end = start + dimension as usize;
    (start..end)
        .map(|value_idx| embedding_values.value(value_idx))
        .collect::<Vec<_>>()
}

fn build_memory_embedding_array(
    records: &[MemoryRecord],
    source_dimension: i32,
    target_dimension: i32,
) -> Result<ArrayRef, AppError> {
    let mut values = Vec::with_capacity(records.len() * target_dimension as usize);
    let mut nulls = Vec::with_capacity(records.len());

    // Arrow NullBuffer convention: `true` = valid, `false` = null.
    for record in records {
        let use_embedding = source_dimension == target_dimension && record.embedding.is_some();
        if use_embedding {
            let embedding = record.embedding.as_ref().expect("checked is_some");
            if embedding.len() != target_dimension as usize {
                return Err(AppError::Validation(format!(
                    "Memory embedding length {} does not match target dimension {}",
                    embedding.len(),
                    target_dimension
                )));
            }
