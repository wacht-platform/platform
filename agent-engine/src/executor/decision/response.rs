...
impl AgentExecutor {
    ...
    pub(super) fn map_response_attachments(
        attachments: Option<&Vec<dto::json::agent_executor::ResponseAttachment>>,
    ) -> Option<Vec<ConversationAttachment>> {
        attachments
            .map(|items| {
                items
                    .iter()
                    .filter_map(|attachment| {
                        ...
                        Some(ConversationAttachment {
                            ...
                            attachment_type: match attachment.attachment_type {
                                ResponseAttachmentType::File => ConversationAttachmentType::File,
                                ResponseAttachmentType::Folder => {
                                    ConversationAttachmentType::Folder
                                }
                            },
                        })
                    })
    ...
    pub(crate) fn next_step_decision_key(next_step: &NextStep) -> &'static str {
        match next_step {
            NextStep::Steer => "steer",
            NextStep::SearchTools => "searchtools",
            NextStep::LoadTools => "loadtools",
            NextStep::StartAction => "startaction",
            NextStep::ContinueAction => "continueaction",
            NextStep::EnableLongThink => "enablelongthink",
            NextStep::Abort => "abort",
        }
    }
